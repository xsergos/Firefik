...
func (s *GRPCServer) handleEvent(ev *pb.AgentEvent) {
	...
	case *pb.AgentEvent_Log:
		if k.Log == nil || k.Log.Agent == nil {
			return
		}
...
func toNativeLogLine(in *pb.LogLine) LogLine {
	out := LogLine{
		Agent:  toNativeIdentity(in.Agent),
		Level:  in.Level,
		Source: in.Source,
		Line:   in.Line,
		Fields: copyLabels(in.Fields),
	}
	if in.At != nil {
		out.At = in.At.AsTime()
	}
...
func agentIDFromEvent(ev *pb.AgentEvent) (string, bool) {
	...
	case *pb.AgentEvent_Log:
		if k.Log != nil && k.Log.Agent != nil {
			return k.Log.Agent.InstanceId, k.Log.Agent.InstanceId != ""
		}
	}
	return "", false
}
...
func toNativeSnapshot(in *pb.AgentSnapshot) AgentSnapshot {
	...
	for _, b := range in.Traffic {
		out.Traffic = append(out.Traffic, TrafficBucket{
			Timestamp: b.Ts,
			Accepted:  b.Accepted,
			Dropped:   b.Dropped,
		})
