...
func (r *CertRenewer) Run(ctx context.Context) error {
	if r.Endpoint == "" || r.CertPath == "" || r.KeyPath == "" {
		return nil
	}
	logger := r.logger()
	interval := r.Interval
...
func (r *CertRenewer) tick(ctx context.Context, logger *slog.Logger) {
	...
	client, err := NewRenewClientWithOptions(r.Endpoint, r.CertPath, r.KeyPath, r.CAPath, r.Insecure)
	if err != nil {
		logger.Warn("cert renew: build client", "error", err)
		AgentCertRenewFailedTotal.WithLabelValues("build_client").Inc()
		return
	}
	resp, err := client.Renew(ctx, RenewRequest{AgentID: r.AgentID, TTLSeconds: r.TTLSeconds, CSRPEM: string(csrPEM)})
	if err != nil {
		logger.Warn("cert renew: request failed", "error", err)
		AgentCertRenewFailedTotal.WithLabelValues("http_error").Inc()
		return
	}
	if resp.CertPEM == "" {
	...
	if err := writeFileAtomic(r.CertPath, []byte(resp.CertPEM), 0o644); err != nil {
		logger.Warn("cert renew: write cert", "error", err)
		AgentCertRenewFailedTotal.WithLabelValues("write_cert").Inc()
		return
	}
	if r.BundlePath != "" && resp.BundlePEM != "" {
		if err := writeFileAtomic(r.BundlePath, []byte(resp.BundlePEM), 0o644); err != nil {
	...
	logger.Info("cert renew: rotated",
		"agent_id", r.AgentID,
		"serial", resp.Serial,
		"expires_at", time.Unix(resp.NotAfterUnix, 0).UTC().Format(time.RFC3339),
	)
	if r.OnRotated != nil {
		r.OnRotated()
	}
