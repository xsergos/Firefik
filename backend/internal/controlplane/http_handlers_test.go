...
func TestOperatorFingerprint_PrefersSSOHeaders(t *testing.T) {
	cases := []string{"X-Auth-Subject", "X-Forwarded-User", "X-Auth-Request-User", "X-Forwarded-Email"}
	for _, h := range cases {
		req := httptest.NewRequest(http.MethodPost, "/", nil)
		req.Header.Set("Authorization", "Bearer x")
		req.Header.Set(h, "alice@corp")
		if got := operatorFingerprint(req); got != "alice@corp" {
			t.Errorf("header %s: got %q want alice@corp", h, got)
		}
	}
}

func TestOperatorFingerprint_FallsBackToFingerprintAtIP(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/", nil)
	req.Header.Set("Authorization", "Bearer secret")
	req.RemoteAddr = "10.0.0.5:54321"
	got := operatorFingerprint(req)
	if !strings.HasPrefix(got, "fp:") || !strings.HasSuffix(got, "@10.0.0.5") {
		t.Errorf("got %q want fp:...@10.0.0.5", got)
	}
}

func TestOperatorFingerprint_RespectsXForwardedFor(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/", nil)
	req.Header.Set("Authorization", "Bearer secret")
	req.Header.Set("X-Forwarded-For", "203.0.113.7, 10.0.0.1")
	got := operatorFingerprint(req)
	if !strings.HasSuffix(got, "@203.0.113.7") {
		t.Errorf("got %q want suffix @203.0.113.7", got)
	}
}

func TestOperatorFingerprint_AnonymousAtIP(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/", nil)
	req.RemoteAddr = "127.0.0.1:8080"
	got := operatorFingerprint(req)
	if got != "anonymous@127.0.0.1" {
		t.Errorf("got %q", got)
	}
}

func TestRequestClientIP_BareRemoteAddr(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	...
	req2.RemoteAddr = ""
	if got := requestClientIP(req2); got != "" {
